use std::{fs::{self, File}, io::Read, path::{Path, PathBuf}, time::Duration};
...
fn hash_file(path: &Path) -> Result<(String, u64)> {
    ...
    loop {
        ...
        length = length.checked_add(read as u64).ok_or_else(|| anyhow!("Photoshop 版本文件过大"))?;
    }
...
pub fn create_version(state: &AppState, request: CreatePhotoshopVersion) -> Result<ProjectCommitResult> {
    ...
    let (sha256, archive_length) = hash_file(Path::new(&archive_path))?;
    ...
    let version = PhotoshopVersionRecord {
        ...
        byte_length: archive_length, sha256: sha256.clone(), blob_id: Some(sha256.clone()), archive_entry: None,
    ...
    let source = BlobSource {
        ...
        byte_length: archive_length, kind: "photoshop-version".into(),
        mime_type: Some("image/vnd.adobe.photoshop".into()),
    };
    let mut project = state.project.lock();
    let mut next_metadata = project.current_metadata().unwrap_or(metadata);
